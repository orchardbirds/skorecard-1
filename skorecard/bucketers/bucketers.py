...
class FrequencyCategoricalBucketer(BaseBucketer):
        """
        The OrdinalCategoricalEncoder() replaces categories by ordinal numbers.
        ...
        def fit(self, X, y=None):
            ...
            for var in self.variables:
                normalized_counts = X[var].value_counts(normalize=True)
                # Remove less frequent categories
                normalized_counts_ok = normalized_counts[normalized_counts >= self.tol]

                ### Bins to merge together
                normalized_counts_merge = normalized_counts[normalized_counts < self.tol]

                # Determine Ordinal Encoder based on ordered labels
                # Note we start at 1, to be able to encode missings as 0.
                last_index_ok = len(normalized_counts_ok) + 1
                mapping = dict(zip(normalized_counts_ok.index, range(1,last_index_ok )))
                for ix  in normalized_counts_merge.index:
                    mapping[ix] =last_index_ok
